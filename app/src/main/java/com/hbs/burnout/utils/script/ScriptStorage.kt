...
class ScriptStorage @Inject constructor() {
    ...
    private val mission2 = listOf(
        ...
        ScriptBuilder(0, "2번째 미션은 내가 알려주는 것을 그려보는 거야.", 0, 2, 2).create(),
        ScriptBuilder(0, "한번 봐보지 않겠어?", 0, 2, 3).create(),
        ScriptBuilder(1, "", 1, 2, 6).addAnswer(
            mapOf(
                0 to "좋아. 뭘 그리면 되지?",
                1 to "흠~~ 귀찮은데~~"
            )
        ).create(),
        ScriptBuilder(0, "그림판을 줄게 그려될 것은 그림판에서 알려줄거야 잘그렸는지 알려 줄게.", 5, 2, 5).addAnswer(
            mapOf(
                0 to "그림판을 줄게 거기에 그려봐 내가 잘그렸는지 알려 줄게.",
                1 to "아주 쉬운거야 대충 그려도 괜찮아."
            )
        ).create(),
        Script(0, "그림을 다그리면 나에게 말을 걸어줘", 0, 2, 4),
        Script(1, "잘 그렸잖아!!", 5, 2, 7),
    ...
    private val mission3 = listOf(
        Script(0, "3번째 미션에 도착했어.", 0, 3, 0),
        ScriptBuilder(1, "", 1, 3, 1).addAnswer(
            mapOf(
                0 to "좋아. 뭐지?",
                1 to "..."
            )
